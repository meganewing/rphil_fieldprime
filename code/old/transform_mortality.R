...
# Output file path. Set to NULL to auto-generate (same name as input + "_transformed.csv")
...
transform_mortality <- function(input_file, output_file = NULL) {
  ...
  #    Each block starts with a row whose col-1 matches e.g. "4HR", "29.5HR"
  # ------------------------------------------------------------------
  hr_pattern <- "^[0-9]+\\.?[0-9]*HR$"
  timepoint_blocks <- list()
  for (i in seq_len(nrows)) {
    val <- mat[i, 1]  # keep original case for column naming
  ...
  # (the same label repeats across plates in each block, but we only need one)
  seen_hrs <- c()
  unique_blocks <- list()
  ...
  tp_cols <- sapply(unique_blocks, `[[`, "hr")  # output column names, file order
  ...
  get_well_cols <- function(plate_col) {
    ...
    for (offset in 1:7) {
      ...
      well_label <- sub("\\.0$", "", cell)  # strip trailing .0 from "2.0" etc.
  ...
  meta_cols  <- c("PlateID_well", "ReplicatePlate", "Immune_Priming", "Heat_Priming", "Assay_Temp")
  ...
  for (pl in plates) {
    ...
    for (row_label in ROW_LABELS) {
      ...
      for (wc in well_cols) {
        ...
        rec <- list(
          "PlateID_well"              = cup_id,
          "ReplicatePlate"                 = parsed$replicate,
